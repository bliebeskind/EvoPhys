...
class ModelInference(object):
	...
	def __ln_posterior(self, theta, x, y):
	    ...
	    if not np.isfinite(lp):
	        return -np.inf
	    return lp + self._ln_likelihood(theta, x, y)

	def sample(self, iterations=1000, n_walkers=100, burnin=100):
		# Set up the sampler.
		...
		res = sampler.run_mcmc(pos, iterations, rstate0=np.random.get_state())
